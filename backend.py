...
MENU_STRUCTURE = {
   "main": {
    "prompt": """Welcome to Air India IVR System.
You can press any number from 0 to 9.
Press 1 for Booking.
Press 2 for Flight Status.
Press 3 for Baggage Services.
Press 4 for Refunds & Cancellations.
Press 5 for Loyalty Program.
Press 6 for Flight Offers and Discounts.
Press 7 for Airport & Lounges Info.
Press 8 for Travel Guidelines.
Press 9 to Speak to an Agent.
Press 0 to repeat this menu.""",

    "options": {
        "0": {"action": "repeat_menu", "message": "Repeating main menu."},
        "1": {"action": "goto_menu", "target": "booking", "message": "Opening booking menu."},
        "2": {"action": "goto_menu", "target": "flight_status", "message": "Opening flight status menu."},
        "3": {"action": "baggage_services", "message": "Redirecting to baggage services."},
        "4": {"action": "refund_help", "message": "Connecting you to refund and cancellation support."},
        "5": {"action": "loyalty_program", "message": "Accessing loyalty program details."},
        "6": {"action": "offers_discounts", "message": "Fetching latest offers and discounts."},
        "7": {"action": "airport_lounges", "message": "Airport and lounge information."},
        "8": {"action": "travel_guidelines", "message": "Providing travel guidelines."},
        "9": {"action": "transfer_agent", "message": "Connecting you to an agent."}
    }
},

    "booking": {
    "prompt": """Booking Menu.
Press 1 for Domestic Tickets.
Press 2 for International Tickets.
Press 3 to Modify Booking.
Press 4 to Cancel Booking.
Press 5 for Seat & Meal Selection.
Press 6 for Student & Senior Offers.
Press 7 for Group Travel.
Press 8 for Travel Insurance.
Press 9 to Talk to a Booking Agent.
Press 0 for Main Menu.""",
    ...
    "options": {
        ...
        "3": {"action": "modify_booking", "message": "Please provide booking ID to modify."},
        "4": {"action": "cancel_booking", "message": "Cancellation process started."},
        "5": {"action": "seat_meal", "message": "Seat and meal selection assistance."},
        "6": {"action": "special_offers", "message": "Applying student/senior discounts."},
        "7": {"action": "group_travel", "message": "Transferring to group travel desk."},
        "8": {"action": "travel_insurance", "message": "Travel insurance support."},
        "9": {"action": "transfer_agent", "message": "Connecting you to booking agent."}
    }
},
"flight_status": {
    "prompt": """Flight Status Menu.
You may speak or enter your 6-digit PNR.
Press 1 for Departure Status.
Press 2 for Arrival Status.
Press 3 for Delay Information.
Press 4 for Boarding Gate Info.
Press 5 for Baggage Belt Info.
Press 6 for Alternate Flight Options.
Press 7 for Route Map.
Press 8 for Travel Advisory.
Press 9 to Speak to a Flight Support Agent.
Press 0 for Main Menu.""",

    "options": {
        "0": {"action": "goto_menu", "target": "main", "message": "Returning to main menu."},
        "1": {"action": "flight_departure", "message": "Fetching departure status."},
        "2": {"action": "flight_arrival", "message": "Fetching arrival status."},
        "3": {"action": "delay_info", "message": "Retrieving delay information."},
        "4": {"action": "gate_info", "message": "Checking gate information."},
        "5": {"action": "baggage_belt", "message": "Retrieving baggage belt details."},
        "6": {"action": "alternate_flights", "message": "Searching for alternate flights."},
        "7": {"action": "route_map", "message": "Fetching route info."},
        "8": {"action": "travel_advice", "message": "Providing travel advisory."},
        "9": {"action": "transfer_agent", "message": "Connecting you to flight support agent."}
    }
},
...
# Store mapping between Legacy VXML Calls and AI Call Sessions
legacy_call_map = {}  # legacy_call_id -> call_id (AI)

# -----------------------------------
# 1) VXML -> Middleware Hook (legacy IVR pushing events)
# -----------------------------------
...
@app.post("/vxml/event")
async def vxml_event_handler(req: VXMLRequest):
    """
    Legacy VXML pushes events here.
    This bridges OLD IVR to AI layer.
    """
    # Map or create AI session for legacy call
    if req.legacy_call_id not in legacy_call_map:
        ai_call_id = make_call_session(caller="Legacy_IVR")
        legacy_call_map[req.legacy_call_id] = ai_call_id
    else:
        ai_call_id = legacy_call_map[req.legacy_call_id]

    # Route event to conversational engine
    response = await handle_transcribed_text(ai_call_id, req.data or "")

    return {"status": "ok", "ai_call_id": ai_call_id, "ai_response": response}


# -----------------------------------
# 2) ACS Connector - Initiate ACS call & send media
# -----------------------------------
ACS_ENDPOINT = os.getenv("ACS_ENDPOINT")
ACS_KEY = os.getenv("ACS_KEY")


async def send_to_acs_tts(call_id: str, text: str):
    """
    Send TTS to ACS call (placeholder).
    Replace with Azure SDK call automation.
    """
    if not ACS_ENDPOINT or not ACS_KEY:
        print("[ACS] Skipped - ACS not configured")
        return
    async with httpx.AsyncClient() as client:
        await client.post(
            f"{ACS_ENDPOINT}/playaudio",
            headers={"x-acs-key": ACS_KEY},
            json={"call_id": call_id, "text": text}
        )


# Modify TTS function to trigger ACS bridge too
async def play_tts_to_call(call_id: str, text: str) -> None:
    print(f"[TTS] to {call_id} => {text}")

    # stream to ACS
    await send_to_acs_tts(call_id, text)
...
# -----------------------------------
# 3) BAP Connector - Trigger workflow
# -----------------------------------
BAP_WEBHOOK = os.getenv("BAP_WEBHOOK")  # Power Automate / Logic App URL

async def trigger_bap_event(event_name: str, payload: dict):
    """
    Sends event to BAP workflow (Power Automate / Logic Apps).
    """
    if not BAP_WEBHOOK:
        print("[BAP] not configured")
        return

    async with httpx.AsyncClient() as client:
        await client.post(BAP_WEBHOOK, json={"event": event_name, "data": payload})


@app.post("/ivr/integration_test")
async def integration_test():
    ...
    - BAP event emit
    """
    test_call = make_call_session("integration-test-user")
    await trigger_bap_event("ivr_test_start", {"call_id": test_call})
    await play_tts_to_call(test_call, "Integration layer test successful.")
    return {"status": "passed", "call_id": test_call}
